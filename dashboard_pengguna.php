...
$id_pemilih = isset($_SESSION['id_pemilih']) ? $_SESSION['id_pemilih'] : null;
...
<head>
  ...
  <style>
      ...
      .candidate-name {
          font-size: 1.2em; /* Ukuran font untuk nama k ...nama kandidat */
...
.image-container {
    ...
    overflow: hidden; /* Jika Anda ingin menyembunyikan bagian gambar yang terpotong */
}
...
<body class="index-page">
        ...
        <div class="candidates-container">
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    echo "<div class='candidate' onclick='pilihKandidat(" . $row['id_kandidat'] . ")'>";
                    echo "<img src ='get_image.php?id=" . $row['id_kandidat'] . "' alt='" . $row['nama_kandidat'] . "'>";
  ...
  <script>
const id_pemilih = <?php echo json_encode($id_pemilih); ?>; // Mengambil dari PHP
</script>
  <script>

let hasVoted = false; // Variabel untuk menyimpan status pemilihan

  function pilihKandidat(id_kandidat) {
    console.log("ID Kandidat yang dipilih: " + id_kandidat);
    console.log("ID Pemilih: " + id_pemilih); // Debugging
    $.ajax({
        type: "POST",
        url: "proses_suara.php",
        data: { id_kandidat: id_kandidat, id_pemilih: id_pemilih }, // Kirim id_kandidat dan id_pemilih
        dataType: "json",
        success: function(response) {
            if (response.status === "sukses") {
                alert("Suara Anda telah berhasil disimpan!");
...
function showNotification(message) {
    alert(message); // Anda bisa mengganti ini dengan notifikasi yang lebih baik
}
